...
namespace Common.Core.Controls
{
	...
	public class AutoCompleteView : ContentView
	{
		...
		/// </summary>
		public static readonly BindableProperty SelectedItemProperty = BindableProperty.Create<AutoCompleteView, object>(p => p.SelectedItem, null, BindingMode.TwoWay, null, ShowSelectedItemChanged);
		...
		/// </summary>
		public static readonly BindableProperty SuggestionsHeightRequestProperty = BindableProperty.Create<AutoCompleteView, double>(p => p.SuggestionsHeightRequest, 500, BindingMode.TwoWay, null, SuggestionHeightRequestChanged);
		...
		public AutoCompleteView()
		{
			...
			AvailableSuggestions = new ObservableCollectionFast<object>();
			...
			_entText.Focused += onFocused;
			_entText.Unfocused += onUnFocused;
			ShowHideListbox(false);
		}
		...
		private void onFocused(object sender, FocusEventArgs focusEventArgs)
		{
			if (focusEventArgs.IsFocused)
			{
				...
				{
					AvailableSuggestions.AddRangeBottom(Suggestions);
				}
		...
		public ObservableCollectionFast<object> AvailableSuggestions
		{
			get { return _availableSuggestions; }
			set
			{
				_lstSuggestions.ItemsSource = value;
				_availableSuggestions = value;
			}
		}
		private ObservableCollectionFast<object> _availableSuggestions;
		...
		private static void ShowSelectedItemChanged(BindableObject obj, object oldValue, object newValue)
		{
			var autoCompleteView = obj as AutoCompleteView;
			if (autoCompleteView != null && oldValue != newValue && newValue != null)
			{
				_preserveSuggestions = true;
				autoCompleteView._entText.Text = newValue.ToString();
				_preserveSuggestions = false;
			}
		}
		...
		private static bool _preserveSuggestions = false;
		...
		private static void TextValueChanged(BindableObject obj, string oldPlaceHolderValue, string newPlaceHolderValue)
		{
			...
			if (control != null)
			{
				...
				var cleanedNewPlaceHolderValue = (newPlaceHolderValue ?? string.Empty).ToLower();

				if (!string.IsNullOrEmpty(cleanedNewPlaceHolderValue) && control.Suggestions != null)
				{
					var filteredSuggestions = new List<object>();
					control.TextBackgroundColor = Color.White;
					control.SelectedItem = null;
					foreach (var suggestion in control.Suggestions)
					{
						var suggestionString = suggestion.ToString().ToLower();
						if (suggestionString == cleanedNewPlaceHolderValue)
						{
							control.TextBackgroundColor = Color.FromRgb(193, 255, 193);
							control.SelectedItem = suggestion;
						}
						if (suggestionString.Contains(cleanedNewPlaceHolderValue))
						{
					...
					control.AvailableSuggestions = new ObservableCollectionFast<object>();
					if (filteredSuggestions.Count > 0)
					{
						foreach (var suggestion in filteredSuggestions)
						{
							control.AvailableSuggestions.Add(suggestion);
						}
						control.ShowHideListbox(true);
						if (control.SelectedItem == null)
						{
							control.TextBackgroundColor = Color.FromRgb(255, 193, 193);
						}
					}
					else
					{
				...
				else if(string.IsNullOrEmpty(cleanedNewPlaceHolderValue))
				{
					control.SelectedItem = null;
					control.TextBackgroundColor = Color.White;
					control._entText.Text = string.Empty;
				}
				else
				{
